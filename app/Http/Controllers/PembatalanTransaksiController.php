...
class PembatalanTransaksiController extends Controller
{
    public function index(Request $request)
    {
        if ($request->method() == 'GET') {
            $getdata = PembatalanTransaksiModel::select('id', 'kode_document', 'username', 'nama_document', 'created_at', 'updated_at')->where('deleted_at', '=', null)->take(75)->get();
            $kode_doc = session('kode_document');
            $skpt_barang_diterima = DocumentTransaksiModel::select('*')->where('username', 'user_a')->where('jenis_surat', $kode_doc)->get();
        } else {
            if ($request->tanggal_akhir == '') {
                $request->tanggal_akhir = $request->tanggal_mulai;

                $getdata = PembatalanTransaksiModel::select('id', 'kode_document', 'username', 'nama_document', 'created_at', 'updated_at')->where('deleted_at', '=', null)->where('created_at', '>=', $request->tanggal_mulai . ' 00:00:00')->take(75)->get();
            } else {
                $getdata = PembatalanTransaksiModel::select('id', 'kode_document', 'username', 'nama_document', 'created_at', 'updated_at')->where('deleted_at', '=', null)->where('created_at', '>=', $request->tanggal_mulai . ' 00:00:00')->where('created_at', '<=', $request->tanggal_akhir . ' 23:59:59')->take(75)->get();
            }
        ...
        return view('documents.surat_kesepakatan_pembatalan_transaksi.pembatalan_transaksi', [
            'getdata' => $getdata,
            'skpt_barang_diterima' => $skpt_barang_diterima
        ]);
    }
        ...
        if ($request->hasFile('document')) {
            ...
            if ($file->getMimeType() == 'application/pdf') {
                if ($file->getSize() <= 307200) {
                    ...
                    return redirect('/pembatalan_transaksi')->with('add_size_invalid', 'Ukuran PDF minimal 4 MB');
                }
            } else {
    ...
    {
        $get = PembatalanTransaksiModel::select('file', 'nama_document')->where('username', '=', $request->username)->where('kode_document', '=', $request->kode_document)->get()[0];
        // dd($get);
    ...
    {
        if ($request->sorting == 'document_terbaru') {
            $getdata = PembatalanTransaksiModel::select('kode_document', 'username', 'nama_document', 'created_at')->where('deleted_at', '=', null)->orderBy('created_at', 'desc')->take(75)->get();
        } else {
            $getdata = PembatalanTransaksiModel::select('kode_document', 'username', 'nama_document', 'created_at')->where('deleted_at', '=', null)->orderBy('created_at', 'asc')->take(75)->get();
        }
        return $getdata;
    }
